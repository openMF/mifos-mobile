 ...
 */  interface GuarantorService {
    ...
    fun createGuarantor(@Path("loanId") loanId: Long,
                        @Body payload: GuarantorApplicationPayload?): Observable<ResponseBody?>?

    @PUT("loans/{loanId}/guarantors/{guarantorId}")
    fun updateGuarantor(@Body payload: GuarantorApplicationPayload?,
                        @Path("loanId") loanId: Long,
                        @Path("guarantorId") guarantorId: Long): Observable<ResponseBody?>?

    @DELETE("/loans/{loanId}/guarantors/{guarantorId}")
    fun deleteGuarantor(@Path("loanId") loanId: Long,
                        @Path("guarantorId") guarantorId: Long): Observable<ResponseBody?>?
}