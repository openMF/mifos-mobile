...
interface LoanAccountsListService {
    ...
    fun getLoanWithAssociations(
            ...
            @Query("associations") associationType: String?): Observable<LoanWithAssociations?>?
    ...
    fun getLoanTemplateByProduct(
            ...
            @Query("productId") productId: Int?): Observable<LoanTemplate?>?
    ...
    fun updateLoanAccount(@Path("loanId") loanId: Long,
                          @Body loansPayload: LoansPayload?): Observable<ResponseBody?>?

    @POST(ApiEndPoints.LOANS + "/{loanId}?command=withdrawnByApplicant")
    fun withdrawLoanAccount(@Path(Constants.LOAN_ID) loanId: Long,
                            @Body loanWithdraw: LoanWithdraw?): Observable<ResponseBody?>?
}