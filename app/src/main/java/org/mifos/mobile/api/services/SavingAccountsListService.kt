...
interface SavingAccountsListService {
    ...
    fun getSavingsWithAssociations(
            ...
            @Query("associations") associationType: String?): Observable<SavingsWithAssociations?>?
    ...
    fun getSavingsAccountApplicationTemplate(
            @Query("clientId") clientId: Long): Observable<SavingsAccountTemplate?>?

    @POST(ApiEndPoints.SAVINGS_ACCOUNTS)
    fun submitSavingAccountApplication(
            @Body payload: SavingsAccountApplicationPayload?): Observable<ResponseBody?>?

    @PUT(ApiEndPoints.SAVINGS_ACCOUNTS + "/{accountsId}")
    fun updateSavingsAccountUpdate(
            @Path("accountsId") accountsId: String?, @Body payload: SavingsAccountUpdatePayload?): Observable<ResponseBody?>?

    @POST(ApiEndPoints.SAVINGS_ACCOUNTS + "/{savingsId}?command=withdrawnByApplicant")
    fun submitWithdrawSavingsAccount(
            @Path("savingsId") savingsId: String?, @Body payload: SavingsAccountWithdrawPayload?): Observable<ResponseBody?>?
}