...
public class CheckSelfPermissionAndRequest {
    ...
    public static void requestPermissionFragment(final Fragment fragment,
        ...
        if (fragment.shouldShowRequestPermissionRationale(permission)) {
            ...
            // this thread waiting for the user's response! After the user
            // sees the explanation, try again to request the permission.
            new MaterialDialog.Builder().init(fragment.getContext())
                    .setTitle(R.string.dialog_permission_denied)
                    .setMessage(dialogMessageRetry)
                    .setPositiveButton(R.string.dialog_action_re_try,
                            new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    fragment.requestPermissions(new
                                                    String[]{permission},
                                            permissionRequestCode);
                                }
                            })
            ...
            //Requesting Permission, first time to the device.
            PreferencesHelper preferencesHelper = new PreferencesHelper(fragment.
                    getContext());
            if (preferencesHelper.getBoolean(permissionDeniedStatus, true)) {
                preferencesHelper.putBoolean(permissionDeniedStatus, false);
                                ...
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ...
                                        Intent intent =
                                                new Intent(Settings
                                                        .ACTION_APPLICATION_DETAILS_SETTINGS);
                                        Uri uri = Uri.fromParts(fragment.getResources().getString(
                                                R.string.package_name),
                                                fragment.getContext().getPackageName(),
                                                null);
                                        intent.setData(uri);
                                        PackageManager pm = fragment.getContext()
                                                .getPackageManager();
                                        if (intent.resolveActivity(pm) != null) {
                                            ...
                                            Toast.makeText(fragment.getContext(),
                                                    fragment.getString(R.string.
                                                            msg_setting_activity_not_found),
                                                    Toast.LENGTH_LONG).show();
                                        }
                                    }
                                })
