        ...
        if(passcodeStatus == Constants.PASSCODE_NOT_SET){
            ...
            button.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    String storedPassCode = sp.getString(Constants.SP_KEY_PASSCODE , "abcd") ;
                    if(storedPassCode == "abcd")
                    {
                        Toast.makeText(getApplicationContext() , "PASSCODE NOTE SET!!" , Toast.LENGTH_LONG).show();
                    }

                    String currentPasscode = et_passcode.getText().toString();

                    if(currentPasscode.equals(storedPassCode) && currentPasscode.length() == 4){
