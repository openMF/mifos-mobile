...
class AccountsPresenterTest {
    ...
    fun testLoadClientAccounts() {
        ...
        Mockito.verify(accountsView, Mockito.never())
    ...
    fun testLoadClientAccountsFail() {
        ...
        Mockito.verify(accountsView)?.showProgress()
        Mockito.verify(accountsView)?.hideProgress()
        Mockito.verify(accountsView)
            ?.showError(context?.getString(R.string.error_fetching_accounts))
    }

    @Test
    fun testLoadClientSavingsAccount() {
        Mockito.`when`(dataManager?.getAccounts(Constants.SAVINGS_ACCOUNTS))
    ...
    fun testLoadClientLoanAccount() {
        Mockito.`when`(dataManager?.getAccounts(Constants.LOAN_ACCOUNTS))
    ...
    fun testLoadClientShareAccount() {
        Mockito.`when`(dataManager?.getAccounts(Constants.SHARE_ACCOUNTS))
