...
class BeneficiaryApplicationPresenterTest {
    ...
    fun testShowBeneficiaryTemplate() {
        Mockito.`when`(dataManager?.beneficiaryTemplate)
            .thenReturn(Observable.just(beneficiaryTemplate))
    ...
    fun testCreateBeneficiary() {
        Mockito.`when`(dataManager?.createBeneficiary(payload))
    ...
    fun testUpdateBeneficiary() {
        Mockito.`when`(dataManager?.updateBeneficiary(1, updatePayload))
            .thenReturn(Observable.just(mockedResponseBody))
