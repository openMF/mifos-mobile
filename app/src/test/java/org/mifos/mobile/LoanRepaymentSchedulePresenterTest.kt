...
class LoanRepaymentSchedulePresenterTest {
    ...
    fun testLoadLoanAccountDetails() {
        Mockito.`when`(dataManager?.getLoanWithAssociations(Constants.REPAYMENT_SCHEDULE, 29))
        ...
        Mockito.verify(view, Mockito.never())
            ?.showError(context?.getString(R.string.error_fetching_repayment_schedule))
    }
