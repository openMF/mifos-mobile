...
class RecentTransactionsPresenterTest {
    ...
    fun testLoadRecentTransactionsZeroOffset() {
        offset = 0
        Mockito.`when`(dataManager?.getRecentTransactions(offset, limit)).thenReturn(
            Observable.just(
                recentTransaction,
            ),
        )
        ...
        Mockito.verify(view, Mockito.never())
            ?.showErrorFetchingRecentTransactions(context?.getString(R.string.error_recent_transactions_loading))
    }
