...
class SavingAccountTransactionPresenterTest {
    ...
    fun testLoadSavingsWithAssociations() {
        Mockito.`when`(dataManager?.getSavingsWithAssociations(1, Constants.TRANSACTIONS))
            .thenReturn(
        ...
        Mockito.verify(view, Mockito.never())
            ?.showErrorFetchingSavingAccountsDetail(context?.getString(R.string.error_saving_account_details_loading))
    }
