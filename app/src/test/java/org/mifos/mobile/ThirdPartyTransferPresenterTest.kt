...
class ThirdPartyTransferPresenterTest {
    ...
    fun testTransferTemplate() {
        Mockito.`when`(dataManager?.thirdPartyTransferTemplate)
            .thenReturn(Observable.just(accountOptionsTemplate))
    ...
    fun testTransferTemplateFails() {
        ...
        Mockito.verify(view, Mockito.never())
            ?.showThirdPartyTransferTemplate(accountOptionsTemplate)
