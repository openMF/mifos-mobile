...
public class ClientChargePresenterTest {
    ...
    public void testLoadClientCharges() {
        ...
        verify(view, never()).showErrorFetchingClientCharges(context.getString(R.string.error_client_charge_loading));
    }

    @Test
    public void testLoadClientChargesFails() {
        when(dataManager.getClientCharges(1)).thenReturn(Observable.<Page<Charge>>error(new RuntimeException()));

        presenter.loadClientCharges(1);
        verify(view).showProgress();
        verify(view).hideProgress();
        verify(view, never()).showClientCharges(charge.getPageItems());
        verify(view).showErrorFetchingClientCharges(context.getString(R.string.error_client_charge_loading));
    }

    @Test
    public void testLoadLoanAccountCharges() {
        when(dataManager.getLoanCharges(1)).thenReturn(Observable.just(charge.getPageItems()));
    ...
    public void testLoadLoanAccountChargesFails() {
        when(dataManager.getLoanCharges(1)).thenReturn(Observable.<List<Charge>>error(new RuntimeException()));
    ...
    public void testLoadSavingsAccountCharges() {
        when(dataManager.getSavingsCharges(1)).thenReturn(Observable.just(charge.getPageItems()));
    ...
    public void testLoadSavingsAccountChargesFails() {
        when(dataManager.getSavingsCharges(1)).thenReturn(Observable.<List<Charge>>error(new RuntimeException()));
    ...
    public void testLoadClientLocalChargesFails() {
        when(dataManager.getClientLocalCharges()).thenReturn(Observable.<Page<Charge>>error(new RuntimeException()));
