...
public class RegistrationPresenterTest {
    ...
    public void testRegisterUser() throws Exception {
        ...
        verify(view, never()).showError(context.getString(R.string.fail_register));
    }

    @Test
    public void testRegisterUserFails() throws Exception{
        when(dataManager.registerUser(registerPayload)).thenReturn(Observable.<ResponseBody>error(new RuntimeException()));

        presenter.registerUser(registerPayload);
        verify(view).showProgress();
        verify(view).hideProgress();
        verify(view, never()).showRegisteredSuccessfully();
        verify(view).showError(context.getString(R.string.fail_register));
    }
}