...
class HomeViewModel(
    ...
    initialState = HomeState(dialogState = null),
) {
    ...
    init {
        logger.d { "Stored Client ID in preferences: ${userPreferencesRepositoryImpl.clientId.value}" }
    ...
    private fun loadClientAccountDetails() {
        ...
        viewModelScope.launch {
            ...
            homeRepositoryImpl.clientAccounts(clientId ?: 0).catch {
                ...
                when (clientAccounts) {
                    ...
                    is DataState.Success -> {
                        logger.d { "client total details stored ${clientAccounts.data}" }
                        logger.d { "loan total details stored ${clientAccounts.data.loanAccounts}" }
                        logger.d { "savings total details stored ${clientAccounts.data.savingsAccounts}" }
    ...
    private fun getUserDetails() {
        ...
        viewModelScope.launch {
            ...
            homeRepositoryImpl.currentClient(clientId ?: 0).catch {
                ...
                when (client) {
                    ...
                    is DataState.Success -> updateState {
                        logger.d { "Stored clients in preferences: $client" }
    ...
    private fun getUserImage() {
        viewModelScope.launch {
            ...
            homeRepositoryImpl.clientImage(clientId ?: 0).catch {
            }.collect { response ->
                when (response) {
                    is DataState.Error -> updateState {
                        it.copy(
                    ...
                    is DataState.Success -> {
                        ...
                        val encodedString = httpResponse.bodyAsText()
                        val pureBase64Encoded =
                            encodedString.substring(encodedString.indexOf(',') + 1)
                        userPreferencesRepositoryImpl.updateProfileImage(pureBase64Encoded)
    ...
    private fun setUserProfile(image: String?) {
        ...
        if (!base64String.matches(Regex("^[A-Za-z0-9+/=]+$"))) {
            return
        }
...
//    data object HomeCards : HomeAction
}
