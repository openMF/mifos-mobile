...
internal class LoanAccountWithdrawViewModel(
    ...
    initialState = LoanAccountWithdrawState(dialogState = null),
) {

    init {
        val initialLoanId = savedStateHandle.getStateFlow<Long?>(
        ...
        viewModelScope.launch {
            loanRepositoryImp.getLoanWithAssociations(Constants.TRANSACTIONS, state.loanId)
                .catch { error ->
                    ...
                    updateState {
                        it.copy(loanWithAssociations = loanAssociations.data)
                    }
                }
        }
    }
        ...
        viewModelScope.launch {
            ...
            when (
                loanRepositoryImp.withdrawLoanAccount(
                    loanId = state.loanWithAssociations?.loanProductId?.toLong(),
    ...
    override fun handleAction(action: LoanAccountWithdrawAction) {
        when (action) {
            ...
            is LoanAccountWithdrawAction.LoanReasonChanged -> {
                updateState {
                    it.copy(loanReason = it.loanReason)
                }
            }
