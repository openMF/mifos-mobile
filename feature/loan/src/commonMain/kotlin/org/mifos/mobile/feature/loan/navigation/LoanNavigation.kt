...
sealed class LoanNavigation(val route: String) {
    ...
    data object LoanApplication :
        LoanNavigation(route = "$LOAN_APPLICATION_SCREEN_ROUTE/{$LOAN_ID}/{$LOAN_STATE}") {
        fun passArguments(loanId: Long, loanState: LoanState) =
            "$LOAN_APPLICATION_SCREEN_ROUTE/$loanId/$loanState"
    }
    ...
    data object LoanTransaction :
        LoanNavigation(route = "$LOAN_TRANSACTION_SCREEN_ROUTE/{$LOAN_ID}") {
