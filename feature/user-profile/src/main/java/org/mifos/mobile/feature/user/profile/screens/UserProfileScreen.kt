...
import org.mifos.mobile.feature.user.profile.utils.TextDrawable
import org.mifos.mobile.feature.user.profile.utils.UserDetails
import org.mifos.mobile.feature.user.profile.viewmodel.UserDetailUiState
...
private fun UserProfileContent(
    ...
    val accountNumber =
        nullFieldCheck(context.getString(R.string.account_number), client.accountNo, context)
    val activationDate = nullFieldCheck(
        context.getString(R.string.activation_date),
        DateHelper.getDateAsString(client.activationDate),
        context,
    )
    val officeName =
        nullFieldCheck(context.getString(R.string.office_name), client.officeName, context)
    val clientType =
        nullFieldCheck(context.getString(R.string.client_type), client.clientType?.name, context)
    val groups = nullFieldCheck(
        context.getString(R.string.groups),
        getGroups(client.groups, context),
        context,
    )
    val clientClassification = client.clientClassification?.name ?: "-"
    val phoneNumber =
        nullFieldCheck(context.getString(R.string.phone_number), client.mobileNo, context)
...
internal class UserProfileScreenPreviewProvider : PreviewParameterProvider<UserDetailUiState> {
    private val sampleBitmap: Bitmap? = BitmapFactory.decodeResource(null, R.drawable.mifos_logo)
...
private fun UserProfileScreenPreview(
    @PreviewParameter(UserProfileScreenPreviewProvider::class)
